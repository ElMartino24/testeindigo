import Fanfics from "../models/fanfics.js";
...
export const updateEntry = async (req, res, next) => {
  try {
    ...
    const entry = await Fanfics.findById(update.id);
    ...
    const updatedEntry = await Fanfics.findByIdAndUpdate(
      update.id,
      { text: update.text },
      { new: true }
    );
...
export const getEntry = async (req, res, next) => {
  try {
    const entry = await Fanfics.find({}).populate("userId", "username");
...
export const deleteEntry = async (req, res, next) => {
  try {
    ...
    const entry = await Fanfics.findById(deletePost.id);
    ...
    await Fanfics.findByIdAndDelete(deletePost.id);
