import Fragen from "../models/fragen.js";
...
export const updateEntry = async (req, res, next) => {
  try {
    ...
    const entry = await Fragen.findById(update.id);
    ...
    const updatedEntry = await Fragen.findByIdAndUpdate(
      update.id,
      { text: update.text },
      { new: true }
    );
...
export const getEntry = async (req, res, next) => {
  try {
    const entry = await Fragen.find({}).populate("userId", "username");
...
export const deleteEntry = async (req, res, next) => {
  try {
    ...
    const entry = await Fragen.findById(deletePost.id);
    ...
    await Fragen.findByIdAndDelete(deletePost.id);
