import Fanarts from "../models/fanarts.js";
...
export const updateEntry = async (req, res, next) => {
  try {
    ...
    const entry = await Fanarts.findById(update.id);
    ...
    const updatedEntry = await Fanarts.findByIdAndUpdate(
      update.id,
      { text: update.text },
      { new: true }
    );
...
export const getEntry = async (req, res, next) => {
  try {
    const entry = await Fanarts.find({}).populate("userId", "username");
...
export const deleteEntry = async (req, res, next) => {
  try {
    ...
    const entry = await Fanarts.findById(deletePost.id);
    ...
    await Fanarts.findByIdAndDelete(deletePost.id);
